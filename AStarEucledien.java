...
public class AStarEucledien {
    ...
    static double calculateHeuristic(int[][] board) {
        double dist = 0;
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < N; j++) {
                int val = board[i][j];
    ...
    static void aStarSearch(int[][] start, int startX, int startY) {
        ...
        while (!pq.isEmpty()) {
            long nodeStart = System.nanoTime(); // start timing this node
            ...
            if (isGoal(current.board)) {
                ...
                System.out.println("A* (Euclidean): Reached goal in " + current.level + " moves.");
